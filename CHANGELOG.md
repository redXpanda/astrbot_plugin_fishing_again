...
- **🧮 布局高度同步调整**：新增分区后同步更新帮助图自适应高度，避免内容截断或重叠。
- 感谢 [@LovelyToaster](https://github.com/LovelyToaster) 的贡献 ([#3](https://github.com/Akiyo-dayo/astrbot_plugin_fishing/pull/3))

## v2.4.92 (借贷系统安全升级 + 数据库优化 + 商店元数据规范化)

- **⚖️ 借贷系统安全机制 (Handshake)**：
  - **新增借款确认**：发起借款后，借款人必须输入 `确认借款 #ID` 才能收到资金。彻底杜绝恶意塞钱或强制放贷。
  - **二次余额校验**：确认放款时会重新检查放贷人余额，确保金币流向准确，防止产生负数余额。
  - **申请时效性**：借款申请增加了 24 小时有效期，过期自动失效。
- **💸 一键还债逻辑升级 (Smart Repay)**：
  - 新增指令：`一键还债`、`全部还清`、`一键还清`。
  - **智能排序算法**：优先偿还 **系统借款** 以解除限制，剩余资金按 **利率从高到低** 自动分配。
- **⚡ 数据库性能与稳定性优化**：
  - **WAL 模式开启**：数据库启用 Write-Ahead Logging 模式，彻底修复在大并发下高频出现的 `database is locked` 报错。
  - **事务原子化 (Transactions)**：关键经济操作（借款、还款、放贷）已封装为原子事务，确保数据在异常崩溃时仍能保持一致。
- **⚠️ 逾期惩罚与排行榜优化**：
  - **惩罚逻辑微调**：系统逾期后禁止 **转账、博弈、抽卡**，但允许 **市场/商店购买**（方便买饵赚钱还债）。
  - **排行榜净化**：系统虚拟账户 (`SYSTEM`) 已从金币及各项排行榜中剔除，不再占用排名。
- **🛠️ 紧急修复与合规性**：
  - **修复报错**：解决 `sqlite3.Row` 缺失 `.get` 属性导致的确认借款崩溃。
  - **修复外键**：自动初始化 `SYSTEM` 账户，修复 `FOREIGN KEY constraint failed` 报错。
  - **商店规范化**：规范化 `metadata.yaml` 字段，以适配 AstrBot 插件商店审核标准。

## v2.4.9 (借贷系统 - 玩家间借款 + 系统借款 + 逾期惩罚)

- **💰 新增借贷系统**：全新玩家间借款和系统借款功能
  - **玩家间借款**：支持 `借他@用户 金额` / `借她` / `借它`，灵活表达
  - **系统借款**：应急借款功能，额度 = 历史最高金币 × 10%（可配置）
  - **智能还款**：支持 `还他@用户 金额` / `还系统 金额` / `还钱 金额`，自动优先还系统借款
  - **强制收款**：放贷人可强制收取欠款 `收他@用户 [金额]`
  - **查看借条**：`借条` 指令查看所有借贷记录，支持状态标识和倒计时
  - **资金透明**：查询 `金币` / `钱包` / `余额` 时自动显示总欠款和净资产
  
- **⚠️ 逾期惩罚机制**：系统借款逾期将限制游戏
  - 超过还款期限（默认7天）未还清将标记为逾期
  - 禁止参与骰宝游戏（所有下注类型）
  - 禁止参与擦弹游戏
  - 还清后立即解除限制
...
- **🔧 技术优化**：
  - 实时逾期判断，无需定时器，Bot重启不影响
  - 还款优先级：系统借款优先于玩家借款（避免游戏限制）
  - 支持多种还款方式（还系统/还钱/还他@SYSTEM）
  - 所有金币操作都有完整的事务保护
  
- **⚙️ 配置项**：
  - `loan.default_interest_rate`：借贷利率（默认5%）
  - `loan.system_loan_ratio`：系统借款额度比例（默认10%）
  - `loan.system_loan_days`：还款期限（默认7天）

---

## v2.4.8 (拼手气红包算法优化)

- **🧧 拼手气红包算法优化**：优化了红包金额分配逻辑，解决了"第一个人总是领走一大半"的问题
  - 引入微信同款"二倍均值法"：每次随机的上限为 `(剩余金额 / 剩余人数) * 2`
  - 确保红包分配更加均衡和公平，提升用户参与感
  - 修复了极端情况下剩余金额分配异常的问题
